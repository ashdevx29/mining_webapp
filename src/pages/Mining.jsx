...
export default function Mining() {
  ...
  // ==================== REAL-TIME LIVE EARNINGS (6 Decimals) ====================
  useEffect(() => {
    if (!mining.isActive || !mining.startTime) return;

    const interval = setInterval(() => {
      const now = Date.now();
      const elapsed = now - new Date(mining.startTime).getTime();
      const progress = Math.min((elapsed / mining.duration) * 100, 100);
      
      const totalReward = mining.totalReward || 1;
      const earnedSoFar = (progress / 100) * totalReward;

      setMining(prev => ({
        ...prev,
        progress: Math.floor(progress),
        earnedSoFar: parseFloat(earnedSoFar.toFixed(6)), // ← 6 Decimal Places
        canClaim: progress >= 100
      }));
    }, 1000);

    return () => clearInterval(interval);
  }, [mining.isActive, mining.startTime, mining.duration, mining.totalReward, setMining]);
  ...
  const startMining = async () => {
    try {
      ...
      alert(err.response?.data?.message || "Failed to start mining");
    }
  };
  ...
  return (
    <div className="relative min-h-screen overflow-x-hidden" style={{ background: "transparent" }}>
      ...
      <div className="relative z-10 max-w-[430px] mx-auto min-h-screen pb-28">
        ...
        {/* LIVE EARNINGS - 6 DECIMALS */}
        {mining.isActive && (
          <div style={{ textAlign: 'center', padding: '12px 24px 8px' }}>
            <motion.div
              animate={{ scale: [1, 1.08, 1] }}
              transition={{ duration: 1.5, repeat: Infinity }}
              style={{ fontSize: 28, fontWeight: 800, color: '#5fd66a', letterSpacing: '-0.5px' }}
            >
              +{mining.earnedSoFar?.toFixed(6) || '0.000000'} MINE
            </motion.div>
            <div style={{ fontSize: 13, color: '#e8b84b' }}>Earned this session (Live)</div>
          </div>
        )}
          ...
          <div style={{ position: 'relative' }}>
              ...
              {mining.isActive && (
                <>
                  <motion.div
                    ...
                    style={{ position: 'absolute', inset: -20, borderRadius: '50%', border: '2px solid rgba(232,184,75,0.4)', pointerEvents: 'none' }}
                  />
                  <motion.div
                    animate={{ scale: [1, 1.25, 1], opacity: [0.2, 0.05, 0.2] }}
                    transition={{ duration: 2.5, repeat: Infinity, delay: 0.3 }}
                    style={{ position: 'absolute', inset: -40, borderRadius: '50%', border: '2px solid rgba(232,184,75,0.2)', pointerEvents: 'none' }}
                  />
                </>
              )}
            </AnimatePresence>
            ...
            <motion.button
              ...
              style={{
                ...
                boxShadow: mining.canClaim ? '0 0 50px rgba(95,214,106,0.5)' : '0 0 40px rgba(232,184,75,0.4)',
              }}
            >
              <img
                src={coinImg}
                ...
                style={{
                  ...
                  height: 128, 
                  objectFit: 'contain',
                  filter: mining.canClaim ? 'drop-shadow(0 0 22px rgba(95,214,106,0.7))' : 'drop-shadow(0 0 25px rgba(232,184,75,0.6))',
                }}
              />
              ...
              <div style={{
                ...
                background: CARD_BG, 
                border: `1px solid ${mining.canClaim ? '#5fd66a' : '#e8b84b'}`,
                borderRadius: 99, 
                padding: '6px 14px', 
                display: 'flex', 
                alignItems: 'center', 
                ...
                {mining.canClaim ? <Check size={14} color="#5fd66a" /> : 
                 mining.isActive ? <Pickaxe size={14} color="#e8b84b" /> : 
                 <Moon size={14} color="#e8b84b" />}
        ...
        <div style={{ padding: '0 24px 24px' }}>
          <div style={{ padding: "1px", borderRadius: 24, background: GRADIENT_BORDER }}>
            <div style={{ background: CARD_BG, borderRadius: 22, padding: '24px' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
                ...
                <span style={{ fontSize: 15, fontWeight: 800, ...GRADIENT_TEXT }}>
                  {Math.floor(mining.progress || 0)}%
                </span>
              </div>

              <div style={{ height: 10, borderRadius: 99, background: '#ffffff', overflow: 'hidden', marginBottom: 20 }}>
                <motion.div
                  animate={{ width: `${mining.progress || 0}%` }}
                  transition={{ duration: 0.5 }}
                  style={{
                    height: '100%', 
                    borderRadius: 99,
                    background: mining.canClaim ? 'linear-gradient(90deg, #2c5c30, #5fd66a)' : 'linear-gradient(90deg, #a9762a, #e8b84b)',
                  }}
                />
              </div>
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                ...
                <div style={{ textAlign: 'center' }}>
                  <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 4 }}>
                    ...
                    <span style={{ fontSize: 17, fontWeight: 800, color: '#5fd66a' }}>
                      {mining.earnedSoFar?.toFixed(6) || '0.000000'}
                    </span>
                  </div>
                  <div style={{ fontSize: 11, color: '#e8b84b', marginTop: 4 }}>EARNED</div>
                </div>
                <div style={{ textAlign: 'center' }}>
                  <div style={{ fontSize: 17, fontWeight: 800, color: '#e8b84b' }}>1 USDT</div>
...
//   return () => clearInterval(interval);
// }, [mining.isActive, mining.startTime, mining.duration, setMining]);
//   // useEffect(() => {
//   //   if (!mining.isActive || !mining.startTime) return;
...
// //   setShowClaimModal(false);
// //   await loadMiningStatus();   // ← This was missing
// //   setMining(prev => ({ ...prev, isActive: false, canClaim: false }));
// // };
...
//     if (res.data.success) {
//       setShowClaimModal(false);
//       await loadMiningStatus();   // Refresh UI

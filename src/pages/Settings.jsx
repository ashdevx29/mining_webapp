...
function SettingRow({ icon, label, desc, action, value, onToggle, onClick }) {
  return (
    <div 
      style={{ display: 'flex', alignItems: 'center', gap: 14, padding: '16px 0', borderBottom: '1px solid rgba(58,42,18,0.6)' }}
      onClick={onClick}
    >
      <div style={{
...
export default function SettingsScreen() {
  ...
  const handleChangePin = () => window.location.href = '/reset';
  const handleSessions = () => alert("Active Sessions - Coming Soon");

  const handleLogout = async () => {
    try {
      await fetch(`${API_BASE}/settings/logout`, {
        method: 'POST',
        headers: { Authorization: `Bearer ${token}` }
      });
    } catch (e) {}
        ...
        {SECTIONS.map((section) => (
          <div key={section.title} style={{ padding: '0 24px 20px' }}>
            ...
            <div style={{ padding: "1px", borderRadius: 20, background: GRADIENT_BORDER }}>
              <div style={{ background: CARD_BG, borderRadius: 18, padding: '0 20px' }}>
                {section.items.map((item) => (
                  <SettingRow
                    ...
                    onToggle={() => updateSetting(item.key)}
                    onClick={
                      item.key === 'changePin' ? handleChangePin :
                      item.key === 'sessions' ? handleSessions : undefined
                    }
                  />
                ))}
              </div>
...
// Define SECTIONS (add this before return)
